...
import { BpCheckedIcon, BpIcon, DoActionButton, UnstakeButton } from "./styleHook"
...
export default function NFTCard({
  ...
  const setDetail = async (data) => {
    ...
    if (uri !== undefined) {
      ...
      // } catch(err) {
      // }
      // setImage('https://airnfts.s3.amazonaws.com/nft-images/20211016/Xashworld_1_1634350055889.gif')
setImage("https://placehold.co/300")
    }
      ...
      {(filterState === action || filterState === 2) &&
        <div className={action !== 1 ? "nft-card" : "nft-card nft-card-active"}>
          <div className="card-checkbox">
            <button  onClick={()=>props?.setCheckAble(!props?.checkAble)} >
              {props?.checkAble ?
              ...
              {/* <Checkbox
                checked={props.checkAble}
                onChange={props.setCheckAble}
                size="sm"
