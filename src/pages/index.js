...
import { APP_ID, CHAIN_ID, SERVER_URL, SITE_ERROR, SMARTCONTRACT_ABI, SMARTCONTRACT_ABI_ERC20, SMARTCONTRACT_ADDRESS, SMARTCONTRACT_ADDRESS_ERC20 } from '../../config'
...
export default function Home({ headerAlert, closeAlert }) {
  ...
  const setPastNFTs = async () => {
    ...
    const res = await userNFTs.raw;

    
    // .Web3API.account.getNFTs({ chain: 'bsc', address: accounts[0] })
    setUnstakedCnt(res?.result?.length)
    setLoading(false)
  }
  const getNFTLIST = () => {
    setPastNFTs()
